...
class DynamicFragment : BaseFluxFragment<MainStore>() {
     ...
     * 列表页数
     */
    private var page = MainStore.DEFAULT_PAGE
            ...
            addOnItemTouchListener(RecyclerItemClickListener(context, this,
                    object : RecyclerItemClickListener.OnItemClickListener {
                        override fun onItemClick(view: View, position: Int) {
                            context?.toast("$position 个")
                        }
                    }))
        }
        //下拉刷新监听器，设置获取最新一页数据
        refreshLayout?.setOnRefreshListener {
            page = MainStore.DEFAULT_PAGE
            getData(null)
        }
        //显示数据
        rxStore?.eventListLiveData?.observe(this, Observer {
            if (it.size > 0) {
                //适配器添加数据
    ...
    override fun onResume() {
        super.onResume()
        //如果是第一页，需要刷新(排除屏幕旋转)
        if (page == MainStore.DEFAULT_PAGE) {
     ...
     */
    @Subscribe(tags = [MainAction.GET_NEWS_EVENT], sticky = true)
    ...
    fun getData(rxChange: RxChange?) {
        mainActionCreator.getNewsEvent(githubAppStore.getUserName() ?: "", page)
    }
