 ...
 */
class StartFragment : BaseFluxFragment<LoginStore>() {
    @Inject
        ...
        Observable
                ...
                .subscribe {
                    if (TextUtils.isEmpty(githubAppStore.getAccessToken())) {
                        ...
                        baseActionCreator.postLocalChange(StartAction.TO_LOGIN)
                    } else {
                        //有token，跳转主页面（直接发送RxChange到LoginActivity）
                        baseActionCreator.postLocalChange(StartAction.TO_MAIN)
                    }
                }
    }
}
